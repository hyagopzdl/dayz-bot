  ...
  <script>
    ...
    function renderServiceSettings() {
      ...
      if (metrics && state.persistenceMetrics) {
        ...
        '<div class="settings-note" style="margin-top:12px">Safety: Phase 6 changes routing, not production data ownership. The PZ keeps its existing ADM cache/manifest and server-scoped Neon rows. A second server is still blocked until the per-server in-memory state/cache cutover is completed in Phase 7.</div></div>' +
