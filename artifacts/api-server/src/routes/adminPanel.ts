    ...
    function renderServiceSettings() {
      ...
      if (metrics && state.persistenceMetrics) {
        ...
        '<div class="settings-card" style="margin-top:16px"><div class="settings-card-head"><div><h3>Persistence V2 domains</h3><p>Core state is split by responsibility. Background game/runtime changes are coalesced to reduce Neon wake-ups; social, commerce and config remain immediate.</p></div></div>' +
          ...
          '</tbody></table></div>' +
        '</div>' +
        '<div class="settings-card" style="margin-top:16px"><div class="settings-card-head"><div><h3>Multi-server foundation</h3><p>Phase 2 persists only the server registry metadata. Production state, ADM cursors, Nitrado and Discord still use the original single-server path.</p></div></div>' +
        '<div class="diag-grid"><div><span>Phase</span><strong>' + Number(state.serverFoundation?.phase || 1).toLocaleString() + '</strong></div><div><span>Mode</span><strong>' + escapeHtml(String(state.serverFoundation?.mode || 'single-server-compat')) + '</strong></div><div><span>Current server</span><strong>' + escapeHtml(String(state.serverFoundation?.currentServerName || 'PZ Deathmatch')) + '</strong></div><div><span>Server ID</span><strong>' + escapeHtml(String(state.serverFoundation?.currentServerId || 'pz-deathmatch')) + '</strong></div><div><span>Registry persisted</span><strong>' + (state.serverFoundation?.registryPersisted ? 'Yes' : 'No') + '</strong></div><div><span>Registry rows</span><strong>' + Number(state.serverFoundation?.registry?.rowsLoaded || 0).toLocaleString() + '</strong></div><div><span>Data path changed</span><strong>' + (state.serverFoundation?.currentDataPathChanged ? 'Yes' : 'No') + '</strong></div><div><span>Additional servers</span><strong>' + (state.serverFoundation?.additionalServersEnabled ? 'Enabled' : 'Blocked') + '</strong></div></div>' +
        '<div class="member-meta" style="margin-top:10px">Registry table: ' + (state.serverFoundation?.registry?.tableReady ? 'ready' : 'not ready') + ' · Primary seeded: ' + (state.serverFoundation?.registry?.primarySeeded ? 'yes' : 'no') + ' · Loaded: ' + escapeHtml(state.serverFoundation?.registry?.lastLoadedAt ? relativeDate(state.serverFoundation.registry.lastLoadedAt) : 'not yet') + (state.serverFoundation?.registry?.lastError ? ' · Error: ' + escapeHtml(String(state.serverFoundation.registry.lastError)) : '') + '</div>' +
        '<div class="settings-note" style="margin-top:12px">Safety: this deploy adds only a tiny registry table. Existing bot_state IDs, player stats, ADM cursors, Nitrado service and Discord guild remain untouched. New servers still cannot process game data yet.</div></div>' +
