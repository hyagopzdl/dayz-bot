...
router.get(["/app", "/app/rankings", "/app/shop", "/app/shop/category/:categoryId", "/app/shop/item/:itemId", "/app/purchases"], requirePortalAuth, async (req, res) => {
  ...
  const view = req.path.startsWith("/app/rankings") ? "rankings" : req.path.startsWith("/app/purchases") ? "purchases" : req.path.startsWith("/app/shop") ? "shop" : "dashboard";
