...
router.get(["/app", "/app/profile", "/app/players/:gamertag", "/app/rankings", "/app/accounts", "/app/clan", "/app/shop", "/app/shop/category/:categoryId", "/app/shop/item/:itemId", "/app/purchases"], requirePortalAuth, async (req, res) => {
  ...
  const view = req.path.startsWith("/app/players/") ? "public-profile" : req.path.startsWith("/app/profile") ? "profile" : req.path.startsWith("/app/rankings") ? "rankings" : req.path.startsWith("/app/accounts") ? "accounts" : req.path.startsWith("/app/clan") ? "clan" : req.path.startsWith("/app/purchases") ? "purchases" : req.path.startsWith("/app/shop") ? "shop" : "dashboard";
...
router.get("/api/player/accounts", requirePortalAuth, async (req, res) => {
  const state = await getStateAsync();
  res.json(buildPlayerAccountsDashboard(state, req.portalSession!));
});
