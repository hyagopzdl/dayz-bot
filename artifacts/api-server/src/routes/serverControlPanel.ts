...
router.get("/api/servers/control", async (req, res) => {
  ...
  res.json({
    servers: servers.map((server) => ({
      ...
      nitradoServiceId: server.integrations.nitradoServiceId || null,
      discordGuildId: server.integrations.discordGuildId || null,
      services: serviceStatus(server),
      runtimeHealth: (coordinatorByServer.get(server.id) as any)?.health || (server.runtimeEnabled ? "starting" : "stopped"),
