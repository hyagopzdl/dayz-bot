...
import {
  ...
  migrateLegacyAdminServerOwnership,
} from "../lib/adminUsers";
...
import { getOrganizationNitradoCredential } from "../lib/organizationIntegrations";

const router = Router();
const LEGACY_PANEL_COOKIE = "admin_panel_token";
function clearLegacyPanelCookie(res: any) { res.clearCookie(LEGACY_PANEL_COOKIE, { path: "/admin-panel", sameSite: "lax" }); res.clearCookie(LEGACY_PANEL_COOKIE, { path: "/", sameSite: "lax" }); }
...
async function resolveCurrentAdminServerIds(adminUserId: string) {
  ...
  const currentAccess = access.map((entry) => currentById.get(entry.serverId)).filter((server): server is NonNullable<typeof server> => Boolean(server));
  ...
  for (const server of soleOrganizationServers) await grantAdminServerAccess(adminUserId, server.id, "owner");
...
function setupServicesPage(services:any[],error=""){const cards=services.map(s=>`<label class="service"><input type="radio" name="serviceId" value="${esc(s.id)}" required><span><strong>${esc(s.name)}</strong><small>${esc([s.game,s.status].filter(Boolean).join(" · ")||"DayZ")}</small>${s.assignedServerName?`<small>Já reconhecido pelo ADM como ${esc(s.assignedServerName)}</small>`:""}</span></label>`).join("");return shell(`<main class="card"><div class="brand">Advanced DayZ Management</div><div class="progress"><span class="on"></span><span></span></div><div class="step">Etapa 1 de 2</div><h1>Selecione seu servidor</h1><p>Encontramos ${services.length} servidor${services.length===1?"":"es"} DayZ na sua conta Nitrado.</p>${services.length?`<form method="post" action="/admin-panel/setup/server">${cards}<button class="button">Continuar</button></form>`:`<div class="error">Nenhum servidor DayZ foi encontrado para este token.</div>`}<a class="button secondary" href="/admin-panel/setup">Usar outro token</a>${error?`<div class="error">${esc(error)}</div>`:""}</main>`,`Selecionar servidor · ADM`)}
...
router.get("/setup",async(req,res)=>{
  ...
  // A fresh admin with no selected server is intentionally allowed to enter the
  // onboarding flow. Do not resolve the server registry here: there may be no
  // managed server yet, and onboarding itself is responsible for creating it.
  const sessionServerId=String(req.adminSession.serverId||"").trim();
  if(!sessionServerId){res.type("html").send(setupTokenPage(String(req.query.error||"")));return;}
  const server=await resolveManagedServer(sessionServerId);
  if(!server){
    setAdminSessionCookie(req,res,createAdminSession({adminUserId:fresh.id,username:fresh.username,serverId:null}));
    res.type("html").send(setupTokenPage("O servidor desta sessão não está mais disponível. Vamos iniciar o onboarding novamente."));
    return;
  }
  res.type("html").send(discordPage(server.id,req.query.discord==="connected",String(req.query.error||"")));
});
