...
router.get("/", (req, res) => {
  ...
  if (!isSystemOwner(req.adminSession.adminUserId)) return res.status(403).send("System owner access required.");

  res.type("html").send(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ADM · Imagens globais</title><style>
...
</style></head><body><main class="shell"><div class="top"><div><div class="eyebrow">System Owner</div><h1 class="title">Imagens globais do catálogo</h1></div><a class="back" href="/admin-panel">← Voltar ao painel</a></div><section class="card"><div class="search"><input id="q" placeholder="Pesquisar item por nome ou ClassName..." autocomplete="off"><button class="button primary" id="search">Pesquisar</button></div><div class="status" id="status">As imagens definidas aqui valem para todos os servidores.</div><div class="results" id="results"><div class="empty">Pesquise um item para começar.</div></div><div class="override"><h2>Overrides ativos</h2><div id="overrides"><div class="empty">Carregando...</div></div></div></section></main><script>
...
$('search').onclick=search;$('q').addEventListener('keydown',e=>{if(e.key==='Enter')search()});loadOverrides();
</script></body></html>`);
});
...
router.put("/overrides/:className", async (req, res) => {
  try {
    ...
    if (imageUrl.length > 2048 || !/^https?:\/\//i.test(imageUrl)) return res.status(400).json({ error: "imageUrl must be a valid http(s) URL up to 2048 characters." });
