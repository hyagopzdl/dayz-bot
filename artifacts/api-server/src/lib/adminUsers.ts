...
function rowToAdminUser(row: any): AdminUser | null {
  if (!row) return null;
  return { id: String(row.id), username: String(row.username), serverId: row.server_id ? String(row.server_id) : null, active: row.active !== false };
}
...
export async function authenticateAdminUser(usernameInput: unknown, passwordInput: unknown): Promise<AdminUser | null> {
  ...
  const rows = await requireSql()`SELECT id, username, password_hash, server_id, active FROM admin_users WHERE username = ${username} LIMIT 1` as any[];
  const row = rows[0];
...
export async function listAdminOrganizationMemberships(adminUserIdInput: unknown): Promise<AdminOrganizationMembership[]> {
  ...
  const rows = await requireSql()`SELECT admin_user_id, organization_id, role FROM admin_organization_memberships WHERE admin_user_id = ${adminUserId} ORDER BY organization_id` as any[];
  return rows.map((row) => ({ adminUserId: String(row.admin_user_id), organizationId: String(row.organization_id), role: normalizeRole(row.role) }));
}
...
export async function listAdminServerAccess(adminUserIdInput: unknown): Promise<AdminServerAccess[]> {
  ...
  const adminUserId = String(adminUserIdInput || "").trim();
  if (!adminUserId) return [];
  const rows = await requireSql()`SELECT admin_user_id, server_id, organization_id, role FROM admin_server_access WHERE admin_user_id = ${adminUserId} ORDER BY server_id` as any[];
  return rows.map((row) => ({ adminUserId: String(row.admin_user_id), serverId: String(row.server_id), organizationId: String(row.organization_id), role: normalizeRole(row.role) }));
}
...
export async function getAdminServerAccess(adminUserIdInput: unknown, serverIdInput: unknown): Promise<AdminServerAccess | null> {
  ...
  const rows = await requireSql()`SELECT admin_user_id, server_id, organization_id, role FROM admin_server_access WHERE admin_user_id = ${adminUserId} AND server_id = ${serverId} LIMIT 1` as any[];
  const row = rows[0];
  const value = row ? { adminUserId: String(row.admin_user_id), serverId: String(row.server_id), organizationId: String(row.organization_id), role: normalizeRole(row.role) } : null;
...
export async function grantAdminServerAccess(adminUserIdInput: unknown, serverIdInput: unknown, roleInput: unknown = "owner") {
  ...
  await requireSql()`INSERT INTO admin_organization_memberships (admin_user_id, organization_id, role, created_at, updated_at) VALUES (${adminUserId}, ${organizationId}, ${role}, NOW(), NOW()) ON CONFLICT (admin_user_id, organization_id) DO UPDATE SET role = EXCLUDED.role, updated_at = NOW()`;
  await requireSql()`INSERT INTO admin_server_access (admin_user_id, server_id, organization_id, role, created_at, updated_at) VALUES (${adminUserId}, ${serverId}, ${organizationId}, ${role}, NOW(), NOW()) ON CONFLICT (admin_user_id, server_id) DO UPDATE SET organization_id = EXCLUDED.organization_id, role = EXCLUDED.role, updated_at = NOW()`;
...
export async function getAdminTenantDiagnostics() {
  await ensureAdminUsersSchema();
  const rows = await requireSql()`SELECT (SELECT COUNT(*)::int FROM admin_users WHERE active = TRUE) AS admins, (SELECT COUNT(*)::int FROM admin_organization_memberships) AS memberships, (SELECT COUNT(*)::int FROM admin_server_access) AS server_access` as any[];
  const row = rows[0] || {};
  return { admins: Number(row.admins || 0), memberships: Number(row.memberships || 0), serverAccessRows: Number(row.server_access || 0), authorizationModel: "admin-user->organization-membership->server-access", legacySelectedServerColumnRetained: true };
}
