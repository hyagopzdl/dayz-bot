...
async function nitradoOnboardingJson(pathname: string, serverId: string) {
  ...
  try {
    response = await globalThis.fetch(`${NITRADO_API_BASE}${pathname}`, { headers: { Authorization: `Bearer ${token}` } });
  } catch (error) {
    recordNetworkTransfer({ service: "nitrado-onboarding", operation: `GET ${pathname.split("?")[0]}`, direction: "outbound", bytes: 0, ok: false });
    throw error;
  }
...
function findRecursiveField(value: unknown, fieldNames: Set<string>, depth = 0): string | undefined {
  ...
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    if (fieldNames.has(key.toLowerCase())) { const found = firstText(child); if (found) return found; }
    const nested = findRecursiveField(child, fieldNames, depth + 1); if (nested) return nested;
  }
  return undefined;
}
...
export async function testOrganizationNitradoCredential(tokenInput: unknown) {
  ...
  const response = await globalThis.fetch(`${NITRADO_API_BASE}/services`, { headers: { Authorization: `Bearer ${token}` } });
  const text = await response.text();
  recordNetworkTransfer({ service: "nitrado-onboarding", operation: "GET /services credential-test", direction: "outbound", bytes: 0, ok: response.ok });
  recordNetworkTransfer({ service: "nitrado-onboarding", operation: "GET /services credential-test", direction: "inbound", bytes: Buffer.byteLength(text, "utf8"), ok: response.ok });
  ...
  return { validatedAt: new Date().toISOString(), serviceCount: services.length, dayzServiceCount: services.filter((service) => /dayz/i.test(`${service.name} ${service.game || ""}`)).length };
}
...
export async function discoverNitradoServices(serverId: string) {
  ...
  const rawServices: unknown[] = Array.isArray(json?.data?.services) ? json.data.services : Array.isArray(json?.services) ? json.services : Array.isArray(json?.data) ? json.data : [];
  const assignedByServiceId = new Map(listManagedServers().filter((server) => server.id !== serverId && server.integrations.nitradoServiceId).map((server) => [String(server.integrations.nitradoServiceId), { id: server.organizationId === onboardingServer.organizationId ? server.id : "assigned", name: server.organizationId === onboardingServer.organizationId ? server.name : "Outro workspace" }] as const));
  const services = rawServices.map(mapNitradoService).filter((service): service is NitradoServiceOption => Boolean(service)).map((service) => { const assigned = assignedByServiceId.get(service.id); return assigned ? { ...service, assignedServerId: assigned.id, assignedServerName: assigned.name } : service; }).sort((a, b) => { const aDayz = /dayz/i.test(`${a.name} ${a.game || ""}`) ? 1 : 0; const bDayz = /dayz/i.test(`${b.name} ${b.game || ""}`) ? 1 : 0; return bDayz - aDayz || a.name.localeCompare(b.name); }).slice(0, 100);
...
async function listNitradoMissionDirectory(serverId: string, serviceId: string, dir: string, options: { quiet?: boolean } = {}) {
  try {
    const json = await nitradoOnboardingJson(`/services/${encodeURIComponent(serviceId)}/gameservers/file_server/list?dir=${encodeURIComponent(dir)}`, serverId);
    const entries = Array.isArray(json?.data?.entries) ? json.data.entries : [];
    if (!options.quiet) console.log(`[shop-delivery][${serverId}] Nitrado list ok dir=${dir || "/"} entries=${entries.length}`);
    return entries;
  } catch (error) {
    if (!options.quiet) console.warn(`[shop-delivery][${serverId}] Nitrado list failed dir=${dir || "/"}: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
}
...
function deriveNitradoFileRoots(baseDirInput: unknown, details: unknown) {
  ...
  const username = firstText(findRecursiveField(details, new Set(["username", "service_username", "gameserver_username"])), baseDir.match(/\/games\/([^/]+)\//i)?.[1]);
...
export async function validateNitradoServiceSetup(serverId: string, serviceIdInput: unknown, baseDirInput: unknown) {
  ...
  if (!baseDir) throw new Error("Nao foi possivel detectar o base dir automaticamente. Informe o caminho do config e tente validar novamente.");
  if (!/\/noftp\/(?:dayzps|dayzxb)\/config$/i.test(baseDir)) throw new Error("O base dir precisa apontar para o diretorio config de um servidor DayZ (dayzps ou dayzxb).");
  const listJson = await nitradoOnboardingJson(`/services/${encodeURIComponent(serviceId)}/gameservers/file_server/list?dir=${encodeURIComponent(baseDir)}`, serverId);
  ...
  return { serviceId, baseDir, missionDir, detectedBaseDir, looksLikeDayz: /dayz/i.test(gameText), admFilesFound, validatedAt: new Date().toISOString() };
}

export async function listDiscordGuildOptions(serverId: string, requesterDiscordId?: string) {
  const server = assertOnboardingServer(serverId);
  const client = getDiscordClient();
  if (!client.isReady()) return { ready: false, guilds: [] as DiscordGuildOption[], message: "Discord bot ainda nao esta conectado." };
...
export async function listDiscordGuildChannels(serverId: string, guildIdInput: unknown) {
  return listDiscordChannelOptions(serverId, guildIdInput);
}
...
export async function listDiscordChannelOptions(serverId: string, guildIdInput: unknown) {
  ...
  const channels: DiscordChannelOption[] = guild.channels.cache.flatMap((channel) => {
    if (channel.type === ChannelType.GuildCategory) return [{ id: channel.id, name: channel.name, type: "category" }];
    if (channel.type === ChannelType.GuildText) return [{ id: channel.id, name: channel.name, type: "text", parentId: channel.parentId || undefined }];
    return [];
  });
