...
function playerProfileHref(gamertag){return `/app/players/${encodeURIComponent(gamertag)}`;}
function renderLeaderboard(items) {
  ...
  target.innerHTML = items.map((item) => `<div class="leader-row ${item.isCurrentPlayer ? 'current' : ''}"><div class="rank-number">${item.rank}</div><div class="leader-player"><strong><a class="player-profile-link" href="${playerProfileHref(item.gamertag)}">${escapeHtml(item.gamertag)}</a>${item.isCurrentPlayer ? ' · You' : ''}</strong><span>${number.format(item.deaths)} deaths · ${Number(item.kd).toFixed(2)} K/D</span></div><div class="leader-score"><strong>${number.format(item.kills)}</strong><span>KILLS</span></div></div>`).join('');
}
...
const serverSwitcher = byId('serverSwitcher');
serverSwitcher?.addEventListener('change', async () => {
  const nextServerId = serverSwitcher.value;
  if (!nextServerId || nextServerId === document.body.dataset.serverId) return;
  serverSwitcher.disabled = true;
  try {
    const response = await fetch('/api/player/server-context', {
      method: 'POST',
      headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
      body: JSON.stringify({ serverId: nextServerId }),
    });
...
async function renderItem(id){contentRoot.innerHTML='<div class="empty-state"><div class="loader"></div></div>';try{const data=await api(`/api/player/shop/items/${encodeURIComponent(id)}`);selectedItemData=data;contentRoot.innerHTML=`<a class="back-link" href="/app/shop/category/${encodeURIComponent(data.item.category)}">← Back to category</a><section class="item-layout"><article class="item-hero"><div class="item-visual">${imageMarkup(data.item.imageUrl,data.item.name)}</div><div class="item-details"><p class="eyebrow">${escapeHtml(data.item.deliveryKind)}</p><h1>${escapeHtml(data.item.name)}</h1><p>${escapeHtml(data.item.description)}</p>${data.item.kind==="kit"&&data.item.components?.length?'<div class="kit-components"><strong>Includes</strong>'+data.item.components.map(c=>'<div class="summary-row"><span>'+escapeHtml(c.name)+' × '+c.quantity+'</span></div>').join('')+'</div>':''}</div></article><aside class="checkout-card purchase-summary"><h2>Delivery</h2><div class="summary-row"><span>Price</span><strong>${coins(data.item.price)}</strong></div><div class="summary-row"><span>Your balance</span><strong>${coins(data.balance)}</strong></div><div class="summary-row"><span>Delivery</span><strong>Next reset</strong></div><p class="page-subtitle">Choose exactly where your item should be delivered on the Chernarus map.</p><button id="chooseLocation" class="primary-button full-button" ${!data.runtime.canAcceptPurchase||!data.profile.linked||data.balance<data.item.price?'disabled':''}>${data.balance<data.item.price?'Insufficient balance':'Choose delivery location'}</button></aside></section>`;byId('chooseLocation')?.addEventListener('click',openMapDialog);}catch(e){contentRoot.innerHTML=`<div class="empty-state"><p>${escapeHtml(e.message)}</p></div>`;}}
...
async function renderAccounts(){
  ...
  try{
    ...
    if(!data.linked){
      const importData=data.primaryIdentityImport;
      const candidates=Array.isArray(importData?.candidates)?importData.candidates:[];
      const importUi=candidates.length?`<div class="account-import-options"><div><strong>Verified identity found in this workspace</strong><span>${escapeHtml(importData.sourceServer?.name||'Another server')} also knows this gamertag. Importing creates a separate link here; wallet, clans, purchases and stats are never copied.</span></div>${candidates.map(gamertag=>`<button class="primary-button" type="button" data-import-primary="${escapeHtml(gamertag)}">Use ${escapeHtml(gamertag)}</button>`).join('')}</div>`:'';
      contentRoot.innerHTML=`<section class="panel account-onboarding"><p class="eyebrow">Player identity</p><h1>Connect this server</h1><p class="page-subtitle">Choose the gamertag you use on this DayZ server. It must already have appeared in the server ADM logs.</p><form id="linkMainForm" class="clan-form compact" style="max-width:420px;margin-top:20px"><label><span>Main gamertag</span><input id="mainGamertagInput" name="gamertag" required maxlength="32" autocomplete="off" placeholder="Start typing your gamertag"><div id="gamertagSuggestions" class="account-info-box" style="display:none"></div></label><button class="primary-button" type="submit">Link gamertag</button></form>${importUi}<div class="account-info-box"><strong>Isolated by server</strong><span>This link only applies to the currently selected server. Coins, purchases, clans and stats remain completely separate.</span></div></section>`;
      let suggestionTimer;const input=byId('mainGamertagInput');const suggestions=byId('gamertagSuggestions');
      input?.addEventListener('input',()=>{clearTimeout(suggestionTimer);suggestionTimer=setTimeout(async()=>{const q=input.value.trim();if(q.length<2){suggestions.style.display='none';return;}try{const result=await api('/api/player/accounts/gamertags?q='+encodeURIComponent(q));const values=result.gamertags||[];suggestions.innerHTML=values.length?values.map(g=>`<button type="button" class="clan-action" data-gamertag-suggestion="${escapeHtml(g)}">${escapeHtml(g)}</button>`).join(''):'No matching gamertag found yet.';suggestions.style.display='block';document.querySelectorAll('[data-gamertag-suggestion]').forEach(btn=>btn.addEventListener('click',()=>{input.value=btn.dataset.gamertagSuggestion;suggestions.style.display='none';}));}catch{suggestions.style.display='none';}},220);});
      byId('linkMainForm')?.addEventListener('submit',async(e)=>{e.preventDefault();const form=new FormData(e.currentTarget);await submitAccountRequest('/api/player/accounts/link',{method:'POST',body:JSON.stringify({gamertag:form.get('gamertag')})},'Gamertag linked to this server.');});
      document.querySelectorAll('[data-import-primary]').forEach(button=>button.addEventListener('click',async()=>{const gamertag=button.dataset.importPrimary;if(!confirm(`Use ${gamertag} as your main account on this server?`))return;await submitAccountRequest('/api/player/accounts/import-primary',{method:'POST',body:JSON.stringify({gamertag})},'Server identity connected.');}));
      return;
    }
...
function clanMemberRow(member,permissions,currentDiscordId){
  ...
  return `<div class="clan-member-row"><div class="clan-member-main"><a class="clan-avatar player-avatar-link" href="${playerProfileHref(member.gamertag)}">${escapeHtml(member.gamertag.slice(0,2).toUpperCase())}</a><div><strong><a class="player-profile-link" href="${playerProfileHref(member.gamertag)}">${escapeHtml(member.gamertag)}</a>${isSelf?' <span class="you-pill">You</span>':''}</strong><span>${escapeHtml(clanRoleLabels[member.role]||member.role)} · ${number.format(member.kills)} kills · ${Number(member.kd).toFixed(2)} K/D · ${number.format(member.linkedCharacters||1)} linked character${Number(member.linkedCharacters||1)===1?'':'s'}</span></div></div><div class="clan-member-actions">${actions}</div></div>`;
}
...
async function renderClan(){
  ...
  try{
    ...
    if(!data.profile.linked){contentRoot.innerHTML='<section class="panel clan-onboarding"><p class="eyebrow">Clan system</p><h1>Link your DayZ account first</h1><p class="page-subtitle">Open <strong>Accounts</strong> to connect your DayZ identity for this server. Clan membership stays isolated inside the selected server.</p></section>';return;}
...
async function renderPlayerProfile(){
  ...
  try{
    ...
    if(!identity.linked){
      contentRoot.innerHTML=`<section class="panel account-onboarding"><p class="eyebrow">Player profile</p><h1>Connect your DayZ identity</h1><p class="page-subtitle">Open <strong>Accounts</strong> to connect your main gamertag for this server. Your profile will then bring together this server's stats, records, clan and linked characters.</p><a class="primary-button profile-link-cta" href="/app/accounts">Open accounts →</a></section>`;
      return;
    }
...
async function renderPublicPlayerProfile(){
  const parts=location.pathname.split('/').filter(Boolean);const gamertag=parts[2]?decodeURIComponent(parts[2]):'';
  if(!gamertag){contentRoot.innerHTML='<div class="empty-state"><p>Player not found.</p></div>';return;}
  contentRoot.innerHTML='<div class="empty-state"><div class="loader"></div><p>Loading player profile...</p></div>';
  try{
    const data=await api(`/api/player/profiles/${encodeURIComponent(gamertag)}`);const identity=data.identity;const overall=data.stats.overall;const records=data.records;const clan=data.clan;const shot=records.longestShot;
    const avatar=`<div class="profile-hero-avatar fallback">${escapeHtml(String(identity.gamertag||'?').slice(0,2).toUpperCase())}</div>`;
    const clanBadge=clan?`<span class="profile-badge clan">[${escapeHtml(clan.tag)}] ${escapeHtml(clan.name)}</span>`:'';
    contentRoot.innerHTML=`<div class="player-profile-shell"><div class="public-profile-toolbar"><a class="back-link" href="/app/rankings">← Back to rankings</a><span>Public player profile</span></div><section class="profile-hero">${avatar}<div class="profile-hero-copy"><p class="eyebrow">Player profile</p><h1>${escapeHtml(identity.gamertag)}</h1><div class="profile-badges"><span class="profile-badge ${identity.online?'online':''}">${identity.online?'● Online':'○ Offline'}</span>${clanBadge}</div></div><div class="profile-rank-spotlight"><span>Overall rank</span><strong>${overall.rank?`#${number.format(overall.rank)}`:'—'}</strong></div></section><section class="profile-stat-grid">${profileStatCard('Kills',number.format(overall.kills||0))}${profileStatCard('Deaths',number.format(overall.deaths||0))}${profileStatCard('K/D',Number(overall.kd||0).toFixed(2))}${profileStatCard('Current streak',number.format(records.currentStreak||0))}${profileStatCard('Best streak',number.format(records.bestStreak||0))}${profileStatCard('Longest shot',shot?number.format(Math.round(shot.distance)):'—',shot?'m':'')}</section><section class="profile-period-grid">${profilePeriodCard('Overall',data.stats.overall)}${profilePeriodCard('This week',data.stats.weekly)}${profilePeriodCard('Today',data.stats.daily)}</section><section class="profile-content-grid"><article class="panel profile-records"><div class="panel-header" style="padding:0 0 12px"><div><p class="eyebrow">Public records</p><h2>Combat records</h2></div></div><div class="profile-record-list"><div class="profile-record-row"><span>Highest recorded killstreak</span><strong>${number.format(records.bestStreak||0)}</strong></div><div class="profile-record-row"><span>Current killstreak</span><strong>${number.format(records.currentStreak||0)}</strong></div><div class="profile-record-row"><span>Longest shot</span><strong>${shot?`${number.format(Math.round(shot.distance))} m${shot.weapon?` · ${escapeHtml(shot.weapon)}`:''}`:'No record yet'}</strong></div><div class="profile-record-row"><span>Overall position</span><strong>${overall.rank?`#${number.format(overall.rank)}`:'Unranked'}</strong></div></div></article><aside class="panel profile-identity-panel"><p class="eyebrow">Server identity</p><h2>Player details</h2><div class="profile-identity-list"><div class="profile-identity-item"><span>Gamertag</span><strong>${escapeHtml(identity.gamertag)}</strong></div><div class="profile-identity-item"><span>Status</span><strong>${identity.online?'Online':'Offline'}</strong></div><div class="profile-identity-item"><span>Clan</span><strong>${clan?`[${escapeHtml(clan.tag)}] ${escapeHtml(clan.name)}`:'No clan'}</strong></div><div class="profile-identity-item"><span>Clan role</span><strong>${clan?escapeHtml(clan.role):'—'}</strong></div></div></aside></section></div>`;
  }catch(e){contentRoot.innerHTML=`<div class="empty-state"><p>${escapeHtml(e.message)}</p><a class="text-button" href="/app/rankings">Back to rankings</a></div>`;}
}

function killfeedTime(value){const date=new Date(value);if(!Number.isFinite(date.getTime()))return '—';return date.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'});}
function killfeedDistance(value){const distance=Number(value);return Number.isFinite(distance)&&distance>=0?`${number.format(Math.round(distance))} m`:'—';}
async function renderKillFeed(){
  contentRoot.innerHTML=`<div class="killfeed-page"><header class="killfeed-header"><div><p class="eyebrow">Combat activity</p><h1>Kill Feed</h1><p class="page-subtitle">Recent eliminations processed by the server.</p></div><button class="killfeed-refresh" id="killfeedRefresh" type="button">↻ Refresh</button></header><section class="panel killfeed-panel"><div class="killfeed-panel-head"><div><strong>Recent kills</strong><span id="killfeedMeta">Loading...</span></div></div><div id="killfeedRows"><div class="empty-state compact"><div class="loader"></div><p>Loading kill feed...</p></div></div></section></div>`;
  const load=async()=>{const rows=byId('killfeedRows');const meta=byId('killfeedMeta');const button=byId('killfeedRefresh');if(button)button.disabled=true;try{const data=await api('/api/player/killfeed?limit=99');meta.textContent=`${number.format(data.retained||0)} recent events retained${data.updatedAt?` · updated ${relativeTime(data.updatedAt)}`:''}`;if(!data.events?.length){rows.innerHTML='<div class="empty-state compact"><p>No kills recorded yet.</p></div>';return;}rows.innerHTML=data.events.map(event=>`<article class="killfeed-row"><div class="killfeed-event"><span class="killfeed-dot">☠</span><div class="killfeed-copy"><div><a class="killfeed-player killer" href="${playerProfileHref(event.killer)}">${escapeHtml(event.killer)}</a><span class="killfeed-verb"> killed </span><a class="killfeed-player victim" href="${playerProfileHref(event.victim)}">${escapeHtml(event.victim)}</a></div></div></div><div class="killfeed-facts"><span class="killfeed-fact weapon">${escapeHtml(event.weapon||'Unknown')}</span><span class="killfeed-fact distance">${killfeedDistance(event.distance)}</span><time class="killfeed-fact time" datetime="${escapeHtml(event.at)}">${killfeedTime(event.at)}</time></div></article>`).join('');}catch(e){rows.innerHTML=`<div class="empty-state compact"><p>${escapeHtml(e.message)}</p></div>`;}finally{if(button)button.disabled=false;}};
  byId('killfeedRefresh')?.addEventListener('click',load);
  await load();
}

function bootView(){const view=document.body.dataset.view;if(view==='shop'){const parts=location.pathname.split('/').filter(Boolean);if(parts[2]==='category'&&parts[3])renderCategory(decodeURIComponent(parts[3]));else if(parts[2]==='item'&&parts[3])renderItem(decodeURIComponent(parts[3]));else renderShopHome();return;}if(view==='purchases'){renderPurchases();return;}if(view==='profile'){renderPlayerProfile();return;}if(view==='public-profile'){renderPublicPlayerProfile();return;}if(view==='rankings'){renderRankings();return;}if(view==='killfeed'){renderKillFeed();return;}if(view==='accounts'){renderAccounts();return;}if(view==='clan'){renderClan();return;}loadDashboard();}
