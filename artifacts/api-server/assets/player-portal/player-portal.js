...
async function renderAccounts(){
  ...
  try{
    ...
    if(!data.linked){
      ...
      const importUi=candidates.length?`<div class="account-import-options"><div><strong>Verified identity found</strong><span>Your ${escapeHtml(importData.sourceServer?.name||'primary server')} identity also exists in this server's stats. Importing creates a separate link here; wallet, clans and economy are not copied.</span></div>${candidates.map(gamertag=>`<button class="primary-button" type="button" data-import-primary="${escapeHtml(gamertag)}">Use ${escapeHtml(gamertag)}</button>`).join('')}</div>`:`<div class="account-info-box"><strong>No verified identity match yet</strong><span>If you use the same gamertag as the primary server, it will become available here after this server has seen it in the ADM stats. Discord linking can also be configured independently for this server later.</span></div>`;
      contentRoot.innerHTML=`<section class="panel account-onboarding"><p class="eyebrow">Player identity</p><h1>Connect this server</h1><p class="page-subtitle">Identity is server-scoped. Linking here never copies coins, clan membership, purchases or stats from another server.</p>${importUi}</section>`;
